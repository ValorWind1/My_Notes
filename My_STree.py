...
class binary_search_tree:
	...
	def insert(self,value):
		if self.root==None:  # first function adding elements to search tree
			self.root=node(value)    # checking if we can insert it at the root
		else:
			self._insert(value,self.root) #_insert() function is recursive + private
	...
	def _insert(self,value,cur_node):
		if value<cur_node.value:
			...
			if cur_node.right_child==None:        # the same as above but checking if right side is empty
	...
	def _print_tree(self,cur_node): # just like our insert function, we will be using recursion+ private function
		if cur_node!=None:
